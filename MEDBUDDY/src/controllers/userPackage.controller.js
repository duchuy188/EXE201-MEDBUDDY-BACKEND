...
exports.getMyActivePackage = async(req, res) => {
    try {
        ...
        // Determine display package (avoid showing 0-price trial as current value)
        let displayPackage = activePackage.packageId;
        try {
            ...
            const paidNonZero = recentPaid.find(p => (p.amount && p.amount > 0) || (p.packageId && p.packageId.price > 0));
            if (paidNonZero && paidNonZero.packageId) {
                displayPackage = paidNonZero.packageId;
            }
        } catch (_) {}
        ...
        res.json({
            ...
            data: {
                package: {
                    _id: displayPackage._id,
                    name: displayPackage.name,
                    price: displayPackage.price,
                    duration: displayPackage.duration,
                    unit: displayPackage.unit,
                    features: displayPackage.features
                },
        ...
        res.json({
            ...
            data: {
                ...
                daysRemaining: user.activePackage.isActive && user.activePackage.endDate ?
                    Math.ceil((user.activePackage.endDate - now().toDate()) / (1000 * 60 * 60 * 24)) : 0,
