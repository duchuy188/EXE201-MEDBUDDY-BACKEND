...
exports.login = async(req, res) => {
    try {
        ...
        const refreshToken = jwt.sign({ userId: user._id, type: 'refresh' },
...
exports.refreshToken = async(req, res) => {
    try {
        ...
        const newRefreshToken = jwt.sign({ userId: user._id, type: 'refresh' },
            process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET, { expiresIn: '7d' }
        );
        ...
        const newAccessToken = jwt.sign({ userId: user._id, role: user.role },
            process.env.JWT_SECRET, { expiresIn: '45m' }
        );
    ...
    try {
        ...
        if (refreshToken) {
            ...
            if (tokenDoc) {
                ...
                await RefreshToken.updateMany({ userId: tokenDoc.userId, isRevoked: false }, { isRevoked: true });
            }
        }
