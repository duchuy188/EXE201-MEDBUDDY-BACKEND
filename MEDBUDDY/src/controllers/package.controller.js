...
exports.addPackage = async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Chỉ admin mới được thêm gói dịch vụ.' });
    }
    const pkg = await Package.create(req.body);
    return res.status(201).json({ message: 'Thêm gói dịch vụ thành công', data: pkg });
  } catch (err) {
    return res.status(500).json({ message: 'Lỗi server', error: err.message });
  }
};
...
exports.deletePackage = async (req, res) => {
  try {
    if (req.user.role !== 'admin') {
      return res.status(403).json({ message: 'Chỉ admin mới được xóa gói dịch vụ.' });
    }
    const { id } = req.params;
